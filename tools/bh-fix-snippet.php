/**
 * Bahrain live fixes — WPCode snippet 3257.
 * WhatsApp stays UAE +971586634710 until a Bahrain number exists.
 * Every call / tel button stays hidden.
 */
	...
	static function () {
		...
		if ( ( is_front_page() || is_home() ) && ! $is_home_path && ! is_paged() && ! is_feed() ) {
			...
			if ( $tpl ) {
				...
				echo '<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>الصفحة غير موجودة</title></head><body><h1>الصفحة غير موجودة</h1><p>عذراً، هذا الرابط غير موجود على موقع ركن التطور البحرين.</p><p><a href="' . esc_url( home_url( '/' ) ) . '">العودة للرئيسية</a> — أو راسلنا واتساب.</p></body></html>';
			}
			exit;
		}
	},
...
function rukn_bh_fix_html( $html ) {
	...
	// Hide / strip every telephone CTA. Keep WhatsApp on the UAE number.
	...
	// Developer Egypt WhatsApp must not look like company contact.
	$html = preg_replace(
	...
	// Normalize remaining company WhatsApp links to the UAE number (temporary).
	$html = preg_replace(
